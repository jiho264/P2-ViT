...
def main():
    ...
    mean_hessian = [
        ...
        0.8191925959786669,
        0.7076886016952384,
        0.024708840222082775,
        0.06145297177505395,
        0.13322631271040494,
        0.06554926888319061,
        0.06175339225459908,
        0.030678026107910893,
        0.24494822213016829,
        0.06636346426025085,
        0.15758525560166742,
        0.04395577998269693,
        0.14552961945368617,
        0.060864547749392026,
        0.08752683209414383,
        0.05799105819299426,
        0.22538750132546922,
        0.06785646981946868,
        0.07478358821405745,
        0.036487501147269154,
        0.07572471890381866,
        0.04584776940321937,
        0.0906965395135412,
        0.052852272764886334,
        0.07057863784461312,
        0.054111013841287636,
        0.10702172109786383,
        0.06730713583013927,
        0.15666245711129553,
        0.062172999291384645,
        0.14509012240011504,
        0.091604835756826,
        0.2623722516111311,
        0.06393236780883862,
    ...
    if args.mixed:
        ...
        for i in range(2 ** len(global_distance)):
            ...
            new_bit_config = (
                [max(bit_choice)]
                + [bit for bit in bit_config for i in range(2)]
                + [random.choice(bit_choice)]
            )
        ...
        for bit_config in bit_list:
            ...
            omega = [
                mean_hessian[i] * select_diastance[i] for i in range(len(FLOPs) - 1)
            ]
        ...
        for i in range(5):
            ...
            val_loss, val_prec1, val_prec5 = validate(
                args, val_loader, model, criterion, device, bit_config
            )
            print("")
        ...
        for evo in range(evo_iter):
            ...
            while True:
                ...
                new_bit = [
                    bit if random.random() < mutate_prob else random.choice(bit_choice)
                    for bit in old_bit
                ]
                ...
                if not model_size > model_constraint and new_bit not in mutate_bit_list:
                    val_loss, val_prec1, val_prec5 = validate(
                        args, val_loader, model, criterion, device, new_bit
                    )
            ...
            while True:
                ...
                if old_bit_1 == old_bit_2:
                    continue
                new_bit = [
                    bit1 if random.random() < crossover_prob else bit2
                    for (bit1, bit2) in zip(old_bit_1, old_bit_2)
                ]
                model_size = sum([FLOPs[i] * new_bit[i] for i in range(len(FLOPs))])
                if (
                    not model_size > model_constraint
                    and new_bit not in crossover_bit_list
                ):
                    val_loss, val_prec1, val_prec5 = validate(
                        args, val_loader, model, criterion, device, new_bit
                    )
